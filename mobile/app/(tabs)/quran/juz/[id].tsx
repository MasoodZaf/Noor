...
// AlQuran Cloud editions for languages blocked on Fawaz CDN
...
const fetchTranslationTexts = async (juzId: number, language: string): Promise<string[]> => {
    ...
    if (alquranEdition) {
        const res = await fetch(`${ARABIC_API}/juz/${juzId}/${alquranEdition}`);
...
export default function JuzReaderScreen() {
    ...
    const arabicCacheRef   = useRef<string[] | null>(null);
        ...
        const loadJuz = async () => {
            ...
            try {
                const [arabicResult, translationResult] = await Promise.allSettled([
                    fetch(`${ARABIC_API}/juz/${juzId}/editions/quran-uthmani`),
                    fetchTranslationTexts(juzId, language),
                ]);

                let uthmaniAyahs: any[] = [];
                if (arabicResult.status === 'fulfilled' && arabicResult.value.ok) {
                    ...
                    uthmaniAyahs = uthmaniEd?.ayahs || [];
                }

                if (uthmaniAyahs.length === 0) throw new Error('Arabic fetch failed');

                const map: Record<number, string> = {};
                uthmaniAyahs.forEach((a: any) => { map[a.surah.number] = a.surah.englishName; });
                setSurahMap(map);

                arabicCacheRef.current   = uthmaniAyahs.map((a: any) => a.text);
                surahInfoCacheRef.current = uthmaniAyahs.map((a: any) => ({
                ...
                const merged = uthmaniAyahs.map((a: any, i: number) => ({
                    id:               `${a.surah.number}_${a.numberInSurah}`,
                    surah_number:     a.surah.number,
                ...
                if (db) {
                    try {
                        ...
                        arabicCacheRef.current    = filtered.map(r => r.text_arabic || '');
    ...
    useEffect(() => {
        ...
        const updateTranslation = async () => {
            ...
            try {
                ...
                const merged = arabicCache.map((arabicText, i) => ({
